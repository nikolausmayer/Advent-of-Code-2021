...
int main(int argc, char* argv[])
{
  ...
  std::vector<unsigned> position_relative_to(scanners.size(), 0);
  std::vector<std::array<int,3>> relative_position(scanners.size(), {0,0,0});
        ...
        for (auto rot{0}; rot < 24 and not match_found; ++rot) {
          ...
          for (const auto& pA: scanners[indexA]) {
            for (const auto& pB: scanners[indexB]) {
              ...
              int matches{0};
              std::remove_cvref_t<decltype(pA)> centershift;
              for (const auto& mpB: scanners[indexB]) {
                const auto rmpB = rotate(rot, mpB);
                decltype(pA) p{rmpB[0] + shift[0],
                               rmpB[1] + shift[1],
                               rmpB[2] + shift[2]};
                if (const auto m{std::find(std::begin(scanners[indexA]),
                                           std::end(scanners[indexA]), p)}; 
                    m != std::end(scanners[indexA])) {
                  ++matches;
                  centershift = {p[0] - rmpB[0],
                                 p[1] - rmpB[1],
                                 p[2] - rmpB[2]};
                }
                if (matches >= 12)
              ...
              if (matches >= 12) {
                std::cout << "merging " << indexB
                          << " into " << indexA << std::endl;
                match_found = true;
                matched[indexB] = true;

                position_relative_to[indexB] = indexA;
                relative_position[indexB] = centershift;
                for (auto pi{0u}; pi < scanners.size(); ++pi) {
                  if (position_relative_to[pi] == indexB) {
                    relative_position[pi] = rotate(rot, relative_position[pi]);
                    relative_position[pi][0] += centershift[0];
                    relative_position[pi][1] += centershift[1];
                    relative_position[pi][2] += centershift[2];
                    position_relative_to[pi] = indexA;
                  }
                }
  ...
  auto maxdist{0};
  for (auto i{0u}; i < scanners.size()-1; ++i) {
    for (auto j{i+1}; j < scanners.size(); ++j) {
      const auto dist{std::abs(relative_position[i][0] - relative_position[j][0]) +
                      std::abs(relative_position[i][1] - relative_position[j][1]) +
                      std::abs(relative_position[i][2] - relative_position[j][2])};
